...
def call_sam_service(
    ...
    os.makedirs(
        os.path.join(output_folder_path, image_path.replace("/", "-").replace("..", "xx")), exist_ok=True
    )
    output_json_path = os.path.join(
        output_folder_path,
        image_path.replace("/", "-").replace("..", "xx"),
    ...
    try:
        ...
        raise RuntimeError(f"Error calling service: {e}") from e
